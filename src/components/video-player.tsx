...
export function VideoPlayer({ video, isActive, onInteraction, getWatchTime }: VideoPlayerProps) {
  ...
  // Extract the real YouTube ID from the src URL (e.g., https://www.youtube.com/embed/VIDEO_ID)
  // This is necessary because video.id might have a suffix (e.g., VIDEO_ID-0) for uniqueness in the list
  const realVideoId = video.src.split('/').pop() || video.id;
  const videoSrc = `${video.src}?autoplay=1&mute=1&controls=0&loop=1&playlist=${realVideoId}&modestbranding=1&showinfo=0&rel=0`;
